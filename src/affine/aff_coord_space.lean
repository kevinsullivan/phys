...
lemma len_zero : length (field_zero K n) = n + 1 := sorry

lemma head_zero : head (field_zero K n) = 0 := sorry
...
def vec_zero : aff_vec K n s := ⟨field_zero K n, len_zero K n, head_zero K n⟩
...
instance : has_add (aff_vec K n s) := ⟨vec_add K n s⟩
...
begin
split,
exact vec_add_left_neg K n s,
exact vec_add_comm K n s,
exact vec_add_assoc K n s,
exact vec_zero_add K n s,
exact vec_add_zero K n s,
end
